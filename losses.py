...
class SuperGlueCriterion(torch.nn.Module):
    ...
    def forward(self, pred_dict, data_dict):
        all_matches = torch.concat([data_dict['e1i'], data_dict['e2i']], dim=0).transpose(0, 1).unsqueeze(0)
        # NOTE: this is different than the SuperGlue paper, which uses all points
        unmatched0 = data_dict['e1j'].squeeze(0)
        unmatched1 = data_dict['e2j'].squeeze(0)
        ...
        for i in range(len(all_matches[0])):
            x = all_matches[0][i][0]
            y = all_matches[0][i][1]
            loss = -torch.log(scores[0][x][y].exp())
            losses.append(loss)  # check batch size == 1 ?
        for p0 in unmatched0:
            loss = -torch.log(scores[0][p0][-1].exp())
            losses.append(loss)
        for p1 in unmatched1:
            loss = -torch.log(scores[0][-1][p1].exp())
            losses.append(loss)
